 ...
 ****************************************************/
#ifndef HA_FONA_LARA_H
#define HA_FONA_LARA_H

#include "includes/HA_FONA_LARAConfig.h"
#include "includes/HA_FONA_LARAExtIncludes.h"
#include "includes/platform/HA_FONA_LARAPlatform.h"
...
class HA_FONA_LARA : public FONAStreamType {
 public:
  HA_FONA_LARA(int8_t r);
  ...
  // Init Config requirements
  boolean initConfig(uint8_t val);

  // Audio Config requirements
  boolean audioConfig(uint8_t val);
  boolean checkAudioConfig(uint8_t val);
  ...
  // APN
  uint8_t getAPN(char *apn);
  boolean setAPN(char apn);

  // LARA GPIO
  boolean setGPIO(uint8_t gpiopin, uint8_t gpiomode, uint8_t gpioval);

  // set Audio output
  boolean setAudio(uint8_t a);
  boolean setVolume(uint8_t i);
  uint8_t getVolume(void);
  boolean playTone_LTE(uint16_t tone, uint16_t length, uint8_t vol, uint8_t path);
  boolean playToolkitTone(uint16_t t, uint16_t len, uint8_t vol, uint8_t path);
  ...
  boolean sendSMS_worked(char *smsaddr, char *smsmsg);
...
class HA_FONA_LARA_3G : public HA_FONA_LARA {

 public:
  HA_FONA_LARA_3G (int8_t r) : HA_FONA_LARA(r) { _type = FONA3G_A; }
